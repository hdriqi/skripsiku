...
for i in range(1,31):
  ...
  for j in range(len(original_file)):
    ...
    if timestamp > 1280509200 and timestamp < 1509469200: 
      label = 0 if float(original_file[j][1]) > float(original_file[j+i][1])*(float(original_file[j+i][1])*0.1) else 1
      # print str(original_file[j][1]) + ' ' + str(float(original_file[j+i][1])*(float(original_file[j+i][1])*0.1)) + ' ' + str(label)
      temp.append(label)
  dataset.append(temp)
  my_file = open('labels/' + file_name, 'w')
  with my_file:
